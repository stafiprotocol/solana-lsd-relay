...
func RemoveEntrustedStakeManager() *cobra.Command {

	var cmd = &cobra.Command{
		Use:   "remove-entrusted-stake-manager",
		Short: "Remove entrusted stakeManager",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			removeEntrustedStakeManagerPubkey := solana.MustPublicKeyFromBase58(cfg.RemoveEntrustedStakeManagerAddress)

			fmt.Println("stack:", stackPubkey)
			fmt.Println("admin:", adminPubkey)
			fmt.Println("feePayer:", feePayerPubkey)
			fmt.Println("removeEntrustedStakeManagerPubkey:", removeEntrustedStakeManagerPubkey)
		Out:
			for {
			...
			instruction := stack.NewRemoveEntrustedStakeManagerInstruction(removeEntrustedStakeManagerPubkey, stackPubkey, adminPubkey).Build()
			instructions := []solana.Instruction{instruction}

			_, err = common.AdminExecuteInstructions("remove entrusted stake manager", rpcClient, instructions, cfg.KeystorePath, feePayerPubkey, adminPubkey, exportTxMessage)
			if err != nil {
				return err
			}
			return nil
