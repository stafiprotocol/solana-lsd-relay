...
func StartCmd() *cobra.Command {
	...
	var cmd = &cobra.Command{
		Use:   "start",
		Short: "Start solana lsd relay(entrusted mode)",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg, err := config.LoadConfig[config.ConfigStartStack](configPath)
			if err != nil {
				return err
			}
		...
		Out:
			...
			t := task.NewTask(cfg.Endpoint, "", cfg.StackAddress, feePayerAccount)
			err = t.Start()
			if err != nil {
				return err
			}
